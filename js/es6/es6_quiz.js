...
let A = array.filter(a => {
  if(a.id%2 === 0) return a;
});
let B = array.filter(b => {
  if(b.id%3 === 0) return b;
});
console.log(A);
console.log(B);
...
// 5. 교집합 구하기 A & B
console.log('5 -------------------------------------------------------------------');
let intersection = A.filter(_val => -1 !== B.indexOf(_val));
console.log(intersection);

// 6. 차집합 구하기 A - B
console.log('6 -------------------------------------------------------------------');
let difference = A.filter(_val => -1 === B.indexOf(_val));
...
let C = array.filter(c => {
  if(c.id%4 === 0) return c;
}).map(c => {
  const newC = {};
  newC[c.id] = c.name;
  return newC;
